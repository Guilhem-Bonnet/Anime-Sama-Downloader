...
def get_episode_choice(episodes, player_choice):
    ...
    while True:
        try:
            episode_input = input(f"\n{Colors.BOLD}Enter episode number(s) (1-{num_episodes}, ranges like 3-7, comma-separated like 1,2,3, or 'all'): {Colors.ENDC}").strip().lower()
            ...
            # Parse episode numbers (supports: 1,2,3 and 1-5 and combinations like 1,3-5,7)
            episode_nums = []
            ...
            for part in parts:
                ...
                if '-' in part:
                    ...
                    try:
                        ...
                        if start > end:
                            start, end = end, start
                        episode_nums.extend(range(start, end + 1))
                    except ValueError:
                        raise ValueError(f"Invalid range format: {part}")
                else:
                    # Single number
                    episode_nums.append(int(part))
            ...
            print_status(f"Invalid input: {e}. Please use format: 1,2,3 or 1-5 or 'all'.", "error")